...
import wandb
import os.path
#from criteria.localitly_regulizer import Space_Regulizer # original
from projector.PTI.criteria.localitly_regulizer import Space_Regulizer # run_all
import torch
from torchvision import transforms
from lpips import LPIPS
from projector.PTI.training.projectors import w_projector
from projector.PTI.configs import global_config, paths_config, hyperparameters
from projector.PTI.criteria import l2_loss
from projector.PTI.models.e4e.psp import pSp
from projector.PTI.utils.log_utils import log_image_from_w
from projector.PTI.utils.models_utils import toogle_grad, load_old_G 


class BaseCoach:
    def __init__(self, data_loader, use_wandb, noise_mode='const'):
    ...
    def forward(self, w, c, noise_mode):


        if w.shape[1]!= self.G.backbone.mapping.num_ws:
            w = w.repeat([1, self.G.backbone.mapping.num_ws, 1])
        if noise_mode == 'const':
            generated_images = self.G.synthesis(w, c, noise_mode=noise_mode)['image']
        else:
            generated_images = self.G.synthesis(w, c)['image']
