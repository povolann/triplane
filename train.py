...
# My own options - Anya
@click.option('--lazy_reg', help='If true, enable lazy regularization.', metavar='BOOL',  type=bool, required=False, default=True, show_default=True) # original True
@click.option('--use_mednerf', help='If true, enable using MedNeRF set up', metavar='BOOL',  type=bool, required=False, default=False, show_default=True)
@click.option('--z_dim', help='# Input latent (Z) dimensionality.', metavar='INT',  type=click.IntRange(min=256), required=False, default=512, show_default=True) # original 512, not 256
...
def main(**kwargs):
    ...
    z_dim = w_dim = opts.z_dim
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=z_dim, w_dim=w_dim, mapping_kwargs=dnnlib.EasyDict()) # Anya: original 512, not 256
    ...
    if c.batch_gpu < c.D_kwargs.epilogue_kwargs.mbstd_group_size: # commented for Mednerf discriminator
    ...
    c.D_kwargs.class_name = 'training.dual_discriminator.DualDiscriminator' # class name updated
    
    c.G_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions.
    c.loss_kwargs.filter_mode = 'antialiased' # Filter mode for raw images ['antialiased', 'none', float [0-1]]
    c.D_kwargs.disc_c_noise = opts.disc_c_noise # Regularization for discriminator pose conditioning # commented for Mednerf discriminator
    ...
    c.D_kwargs.num_fp16_res = opts.d_num_fp16_res # commented for Mednerf discriminator. this is also slightly different in VoxGRAF
    c.D_kwargs.conv_clamp = 256 if opts.d_num_fp16_res > 0 else None # commented for Mednerf discriminator

    if opts.nobench:
        c.cudnn_benchmark = False

    if not opts.lazy_reg: # Turning off lazy regularization
        opts.density_reg = 0
        c.D_reg_interval = None

    # Using MedNeRF set up Anya
    c.G_kwargs.backbone = 'training.networks_stylegan2.Generator'

    if z_dim == 256:
        sr_module = 'training.superresolution.SuperresolutionHybrid2X_256' # w_dim changed to 256 from 512
        rendering_options.update({
        'superresolution_module': sr_module,
        })

    c.G_kwargs.original = True

    if opts.use_mednerf:
        c.G_kwargs.original = True

        c.G_reg_interval = None # Turn off lazy regularization
        c.D_reg_interval = None
        # c.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.RMSprop', alpha=0.99, eps=1e-8)
        # c.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.RMSprop', alpha=0.99, eps=1e-8)
        c.G_opt_kwargs.lr = 0.0005
        c.D_opt_kwargs.lr = 0.0001
        
        c.G_kwargs.backbone = 'training.networks_simple.Generator'

        # c.D_kwargs.mapping_kwargs.num_layers = 2
        # c.D_kwargs.channel_base = 4000
        # c.D_kwargs.channel_max = z_dim

        # # Setting Mednerf discriminator
        # c.D_kwargs.class_name = 'training.dag_discriminator.Discriminator' # added for Mednerf discriminator
        # c.D_kwargs.channel_base = None # commented for Mednerf discriminator
        # c.D_kwargs.channel_max = None # commented for Mednerf discriminator
        # c.D_kwargs.block_kwargs.freeze_layers = None # commented for Mednerf discriminator
        # c.D_kwargs.epilogue_kwargs.mbstd_group_size = None # commented for Mednerf discriminator
        # c.D_kwargs.disc_c_noise = None # Regularization for discriminator pose conditioning # commented for Mednerf discriminator
        # c.D_kwargs.num_fp16_res = None # commented for Mednerf discriminator. this is also slightly different in VoxGRAF
        # c.D_kwargs.conv_clamp = None # commented for Mednerf discriminator


    # Comment from here down if I want to use original StyleGAN2 backbone
    # Now I am trying to implement ProjGAN
    ...
    # c.D_kwargs.mapping_kwargs.num_layers = 2
    # c.D_kwargs.channel_base = 2048
    # c.D_kwargs.channel_max = 64

    # 20.8.2024 - Projected GAN
    #c.G_kwargs.class_name = 'pg_modules.networks_stylegan2.Generator'
    #c.G_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions. But this is not necessary because it is the same in EG3D.
    ...
    desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-projected'
