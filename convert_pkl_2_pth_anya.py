    ...
    for frame_idx in tqdm(range(num_keyframes * w_frames)):
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                # From original gen_video.py entangle == 'camera'
                img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], noise_mode='const')[image_mode][0] # pkl
                img_new = G_new.synthesis(ws=w.unsqueeze(0), c=c[0:1], noise_mode='const')[image_mode][0] # pth
                ...
                imgs.append(torch.cat([img, img_new],dim=2)) # just putting 2 images next to each other
...
# @click.option('--network_pth', help='Network pth ckpt filename', required=False, default='network.pth')
...
def convert(
    ...
    seeds = [0, 1]
    grid = (1, 1)
    name = network_pkl.split('/')[-1].replace('.pkl', '')
    for seed in seeds:
        output = os.path.join(outdir, f'{name}_{seed}.mp4')
        seeds_ = [seed]
        gen_interp_video(G=G, G_new=G_new, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
                         ...
                         truncation_cutoff=truncation_cutoff, cfg=cfg, focal=foc, image_mode=image_mode)
