...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(7680 // training_set.image_shape[2], 1, 6) # 32 -> 4
    #gh = np.clip(4320 // training_set.image_shape[1], 4, 32) 
    gh = np.clip(4320 // training_set.image_shape[1], 1, 6) # 32 -> 4
...
def save_image_grid_my(img, fname, drange, grid_size):
    lo, hi = drange
    img = np.asarray(img, dtype=np.float32)
    img = (img - lo) * (255 / (hi - lo))
    img = np.rint(img).clip(0, 255).astype(np.uint8)
    ...
    # swapping
    img = img.reshape([gw, gh, C, H, W])  # Swap gw and gh dimensions
    img = img.transpose(1, 3, 0, 4, 2)    # Adjust transpose accordingly
...
def training_loop(
    ...
    continue_training       = False,    # Continue training in existing rundir
):
    # Initialize.
    ...
    #D_kwargs.nc = training_set.num_channels # added for Mednerf discriminator
    #D_kwargs.imsize = training_set.resolution # added for Mednerf discriminator
    #D = dnnlib.util.construct_class_by_name(**D_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module # added for Mednerf discriminator
    G_ema = copy.deepcopy(G).eval()

    # Check for existing checkpoint
    ckpt_pkl = None
    if continue_training and os.path.isfile(misc.get_ckpt_path(run_dir)):       # find last pkl file
        ckpt_pkl = resume_pkl = misc.get_ckpt_path(run_dir)

    # Resume from existing pickle.
    # Anya adjusted to load ticks from VoxGRAF and ProjectedGAN.
    if (resume_pkl is not None) and (rank == 0):
        print(f'Resuming from "{resume_pkl}"')
        with dnnlib.util.open_url(resume_pkl) as f:
            resume_data = legacy.load_network_pkl(f)
            if resume_data.get('progress') is None:
                best_fid = 99999
            else:
    ...
    if rank == 0:
        z = torch.empty([batch_gpu, G.z_dim], device=device) # torch.Size([8, 512])
        c = torch.empty([batch_gpu, G.c_dim], device=device) # torch.Size([8, 25])
    ...
    # if D_kwargs['class_name'] == 'training.dual_discriminator.DualDiscriminator':
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
                ...
                phase_real_img = (phase_real_img.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu) # phase_real_img[0].shape: torch.Size([8, 3, 128, 128])
                phase_real_c = phase_real_c.to(device).split(batch_gpu) # phase_real_c[0].shape: torch.Size([8, 25])
                all_gen_z = torch.randn([len(phases) * batch_size, G.z_dim], device=device)
                all_gen_z = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z.split(batch_size)] # all_gen_z[0][0].shape: torch.Size([8, 512])
                all_gen_c = [training_set.get_label(np.random.randint(len(training_set))) for _ in range(len(phases) * batch_size)]
                all_gen_c = torch.from_numpy(np.stack(all_gen_c)).pin_memory().to(device)
                all_gen_c = [phase_gen_c.split(batch_gpu) for phase_gen_c in all_gen_c.split(batch_size)] # all_gen_c[0][0].shape: torch.Size([8, 25])
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            # #forward_cam2world_pose = LookAtPoseSampler.sample(3.14/4, -3.14/2, torch.tensor([0, 0, 0], device=device), radius=500, device=device) # theta (azimuth), phi (polar)
            # #forward_cam2world_pose = LookAtPoseSampler.sample(rotation_angle_radians, elevation_angle_radians, torch.tensor([0, 0, 0], device=device), radius=500, device=device) # theta (azimuth), phi (polar)
            # from camera_utils import get_render_pose
            # forward_cam2world_pose = get_render_pose(radius=10.5, phi=0, theta=45).unsqueeze(0).to(device)
            ...
            # grid_ws = [G_ema.mapping(z, forward_label.expand(z.shape[0], -1)) for z, c in zip(grid_z, grid_c)]
            # out = [G_ema.synthesis(ws, c=c, noise_mode='const') for ws, c in zip(grid_ws, grid_c)] # same poses as above, original
            # out_my = [G_ema.synthesis(ws, forward_label.expand(ws.shape[0], -1), noise_mode='const') for ws, c in zip(grid_ws, grid_c)] # forward poses

            # images = torch.cat([o['image'].cpu() for o in out]).numpy()
            # images_my = torch.cat([o['image'].cpu() for o in out_my]).numpy()
            # images_raw = torch.cat([o['image_raw'].cpu() for o in out]).numpy()
            # images_depth = -torch.cat([o['image_depth'].cpu() for o in out]).numpy()
            # my_dir = os.path.join(run_dir, f'forward images')
            # os.makedirs(my_dir, exist_ok=True)
            # save_image_grid(images, os.path.join(my_dir, f'fakes{cur_nimg//1000:06d}_f.png'), drange=[-1,1], grid_size=grid_size)
            # save_image_grid(images_my, os.path.join(my_dir, f'fakes{cur_nimg//1000:06d}_f_my.png'), drange=[-1,1], grid_size=grid_size)
            # save_image_grid(images_raw, os.path.join(my_dir, f'fakes{cur_nimg//1000:06d}_raw_f.png'), drange=[-1,1], grid_size=grid_size)
            # save_image_grid(images_depth, os.path.join(my_dir, f'fakes{cur_nimg//1000:06d}_depth_f.png'), drange=[images_depth.min(), images_depth.max()], grid_size=grid_size)
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            snapshot_data = dict(G=G, D=D, G_ema=G_ema, augment_pipe=augment_pipe, training_set_kwargs=dict(training_set_kwargs)) # Anya - adjusted, some stuff is commented comparing
            # to the original EG3D code, done same as in ProjectedGAN
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe)]:
                # if module is not None: # original EG3D, but D is not persistence
                #     if num_gpus > 1:
                #         misc.check_ddp_consistency(module, ignore_regex=r'.*\.[^.]+_(avg|ema)')
                #     module = copy.deepcopy(module).eval().requires_grad_(False).cpu() # not working for D because it is not persistence
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            # save best fid ckpt
            snapshot_pkl = os.path.join(os.path.dirname(snapshot_pkl), f'best_model.pkl')
            if rank == 0:
                if stats_metrics.get('fid50k_full', float('inf')) < best_fid:
                    snapshot_data['best_fid'] = best_fid = stats_metrics['fid50k_full']
                    with open(snapshot_pkl, 'wb') as f:
                        dill.dump(snapshot_data, f)
                    
                    # Save as pth only for the best model
                    G_ema_temp = copy.deepcopy(G_ema).eval().requires_grad_(False).cpu()
                    save_dict = {'G_ema': G_ema_temp.state_dict()}
                    snapshot_pth=snapshot_pkl.replace('pkl','pth')
                    print('Save pth to', snapshot_pth and snapshot_pkl)
                    torch.save(save_dict, snapshot_pth)
                    del G_ema_temp
