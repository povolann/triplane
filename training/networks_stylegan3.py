...
def conv2d(*args, **kwargs):
    return spectral_norm(torch.nn.Conv2d(*args, **kwargs))
...
class DownBlockComp(torch.nn.Module):
    def __init__(self, in_planes, out_planes):
        ...
        self.main = torch.nn.Sequential(
            ...
            batchNorm2d(out_planes), torch.nn.LeakyReLU(0.2, inplace=True),
            conv2d(out_planes, out_planes, 3, 1, 1, bias=False),
            batchNorm2d(out_planes), torch.nn.LeakyReLU(0.2)
            )
...
class SEBlock(torch.nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.main = torch.nn.Sequential(  torch.nn.AdaptiveAvgPool2d(4), 
                                    conv2d(ch_in, ch_out, 4, 1, 0, bias=False), Swish(),
                                    conv2d(ch_out, ch_out, 1, 1, 0, bias=False), torch.nn.Sigmoid() )
...
class SimpleDecoder(torch.nn.Module):
    ...
    def __init__(self, nfc_in=64, nc=3):
        ...
        self.main = torch.nn.Sequential(  torch.nn.AdaptiveAvgPool2d(8),
...
class DiscriminatorGRAF(torch.nn.Module):
    def __init__(self, nc=3, ndf=64, imsize=64):
        super(DiscriminatorGRAF, self).__init__()
        self.nc = nc
        ...
        self.down_from_big = torch.nn.Sequential( 
        ...
        for i in range(4):
            ...
            self.dag_heads.append(torch.nn.Sequential(*head))
        self.dag_heads = torch.nn.ModuleList(self.dag_heads)
